...
@pytest.fixture
def video(db):
    v = Video(slug='demoday', titulo='Demo Day - Polo Digital de Mogi das Cruzes', vimeo_id='439637023')
    v.save()
    return v
...
def resp(client, video):
    return client.get(reverse('demoday:video', args=(video.slug,)))


@pytest.fixture
def resp_video_nao_encontrado(client, video):
    return client.get(reverse('demoday:video', args=(video.slug + 'video_nao_existe',)))
